...
quill::Logger* Logger::get() { return logger_; }
...
void LoggerFactory::Init(toml::table& config)
{
    ...
    switch (config["logging"]["log_level"].value_or(5))
    {
        case 0:
    ...
    logs_path_ = config["logging"]["log_dir"].value_or("./Logs");
...
quill::Logger* LoggerFactory::createLogger(const std::string& logger_file)
{
    ...
    auto logger{quill::Frontend::create_or_get_logger(logger_file, std::move(file_sink), pfo_)};
