...
#include "secret_data_manager.hpp"
#include <iostream>
#include <memory>
#include <toml.hpp>
#include <format>
...
int main(int argc, char* argv[]) {
    try {
        ...
        auto logger{Logging::LoggerFactory::GetLogger(cfg["logging"]["filename"].value_or("json_rpc_server.log"))};

        // Конфигурация
        const std::string address{cfg["server_parameters"]["host"].value_or("0.0.0.0")};
        const unsigned short port{static_cast<unsigned short>(cfg["server_parameters"]["port"].value_or(8080))};
        const std::string version{cfg["server_parameters"]["version"].value_or("v1")};
        ...
        LOG_INFO(logger.get(), "Server {}:{} with version: {} on {} threads was started", address, port, version, threads);
        data_manager.register_methods(server.get_registry());
